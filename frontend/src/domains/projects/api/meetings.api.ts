...
import type { Meeting, MeetingInput, MomDocument } from "../types/meetings.types";
...
export const meetingsApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getMeeting: builder.query<Meeting, { projectId: string; meetingId: string }>({
      query: ({ projectId, meetingId }) => `/projects/${projectId}/meetings/${meetingId}`,
    }),
    createMeeting: builder.mutation<Meeting, { projectId: string; body: MeetingInput }>({
    ...
    deleteMeeting: builder.mutation<void, { projectId: string; meetingId: string }>({
    ...
    generateMom: builder.mutation<MomDocument, { projectId: string; meetingId: string }>({
      query: ({ projectId, meetingId }) => ({
    ...
    getMom: builder.query<MomDocument, { projectId: string; momId: string }>({
      query: ({ projectId, momId }) => `/projects/${projectId}/meetings/moms/${momId}`,
    }),
    reviewMom: builder.mutation<MomDocument, { projectId: string; momId: string }>({
    ...
    acknowledgeMom: builder.mutation<MomDocument, { projectId: string; momId: string }>({
      query: ({ projectId, momId }) => ({
